...
export async function POST(req: NextRequest) {
  try {
    ...
    if (action === "forgot_password") {
      ...
      // SUPER ADMIN → email recovery note (password is hashed — do not email raw hash)
      if (role === "super_admin" || user.username === "superadmin") {
        try {
          ...
          await transporter.sendMail({
            ...
            text: `Forgot-password request for Super Admin: ${user.username}\n\nPassword is stored hashed. Use the dashboard change-password or resolve_reset after verifying identity.\n\nIf you did not request this, secure the dashboard immediately.`,
            html: `<p>Forgot-password request for Super Admin: <strong>${user.username}</strong></p><p>Password is stored as a secure hash. Use change password in the app or resolve_reset as Super Admin.</p>`,
          });
        } catch (e: any) {
      ...
      // Other users → store reset request for Super Admin
