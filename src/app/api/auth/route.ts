...
function createToken(user: { id: number; username: string; role: string }) {
...
async function getPermissionsMap(): Promise<Record<string, string[]>> {
  return getJsonSetting("user_permissions", {});
}
  ...
  try {
    ...
    if (action === "update_permissions") {
      ...
      if (Array.isArray(permissions)) {
        ...
        if (targetRole === "super_admin") perms = [...ALL_PERMISSIONS];
        if (myRole === "admin") {
    ...
    if (action === "get_permissions") {
      ...
      const permMap = await getPermissionsMap();
      if (body.userId) {
        const perms = resolvePermissions(Number(body.userId), role, permMap);
    ...
    if (action === "set_permissions") {
      ...
      if (userId && Array.isArray(permissions)) {
        const permMap = await getPermissionsMap();
        permMap[String(userId)] = permissions.filter((p: string) =>
          (ALL_PERMISSIONS as readonly string[]).includes(p)
        );
        await setJsonSetting("user_permissions", permMap);
    ...
    if (action === "set_smtp_assignments") {
      ...
      return NextResponse.json({
        success: true,
        message: "SMTP assignments saved",
      });
    }
      ...
      if (role === "super_admin" || user.username === "superadmin") {
        try {
          ...
          const transporter = nodemailer.createTransport({
            ...
            secure:
              Boolean(account.secure) || Number(account.smtpPort) === 465,
            auth: {
          ...
          await transporter.sendMail({
            ...
            subject:
              "[Email Automation] Super Admin password recovery request",
            text: `Forgot-password request for Super Admin: ${user.username}\n\nPassword is stored as a secure hash. Use Change Password in the dashboard.`,
            html: `<p>Forgot-password request for Super Admin: <strong>${user.username}</strong></p>`,
          });
        } catch (e: any) {
          ...
          return NextResponse.json(
            {
              success: false,
              error: e.message || "Failed to send recovery email",
            },
            { status: 500 }
          );
        }
