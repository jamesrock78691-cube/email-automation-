  ...
  try {
    ...
    if (action === "create_user") {
      ...
      if (myRole !== "super_admin" && newRole === "super_admin") {
        return NextResponse.json({ success: false, error: "Cannot create super admin" }, { status: 403 });
      }
      ...
      return NextResponse.json({ success: true, message: "User created", id: newId });
    }
    ...
    if (action === "update_user") {
      ...
      if (role) {
        let newRole = normalizeRole(role);
        if (myRole !== "super_admin" && newRole === "super_admin") {
          return NextResponse.json({ success: false, error: "Cannot set super admin" }, { status: 403 });
        }
      ...
      if (Object.keys(userUpdates).length) {
        await db.update(users).set(userUpdates).where(eq(users.id, Number(userId)));
      }
      if (dailyLimit !== undefined) {
        const statsMap = await getAgentStatsMap();
        const key = String(userId);
        statsMap[key] = { ...(statsMap[key] || {}), dailyLimit: Number(dailyLimit) || 100 };
        await setJsonSetting("agent_stats", statsMap);
      }
    ...
    if (action === "delete_user") {
      ...
      if (myRole !== "super_admin") {
        return NextResponse.json({ success: false, error: "Only Super Admin" }, { status: 403 });
      }
      const { userId } = body;
      if (!userId) {
        return NextResponse.json({ success: false, error: "userId required" }, { status: 400 });
    ...
    if (action === "change_password") {
      ...
      await db.update(users).set({ passwordHash: hashed }).where(eq(users.id, session.userId));
    ...
    if (action === "record_send") {
      ...
      if (normalizeRole(session.role) === "operator" && Number(userId) !== session.userId) {
        return NextResponse.json({ success: false, error: "Forbidden" }, { status: 403 });
      }
      const statsMap = await getAgentStatsMap();
      const key = String(userId);
      const today = new Date().toISOString().slice(0, 10);
      const cur = statsMap[key] || { totalSent: 0, sentToday: 0, dailyLimit: 100, lastDate: today };
    ...
    if (action === "get_permissions") {
      ...
      const perms = await getJsonSetting(`permissions_${role}`, DEFAULT_BY_ROLE[role] || []);
    ...
    if (action === "set_permissions") {
      ...
      if (!role || !Array.isArray(permissions)) {
        return NextResponse.json({ success: false, error: "role and permissions required" }, { status: 400 });
      }
    ...
    if (action === "forgot_password") {
      ...
      const genericOk = "If this account exists, a recovery action was triggered. Contact Super Admin if needed.";
      if (!found.length) {
      ...
      if (role === "super_admin" || user.username === "superadmin") {
        try {
          ...
          if (!accounts.length) {
            return NextResponse.json({
              success: false,
              error: "No enabled SMTP account to send recovery email. Add one first.",
            }, { status: 500 });
          }
          ...
          await transporter.sendMail({
            ...
            subject: "Super Admin password recovery",
            text: `Forgot-password request for Super Admin: ${user.username}\n\n(Password is hashed; use change password after login or resolve from admin panel.)\n\nIf you did not request this, secure the dashboard immediately.`,
            html: `<p>Forgot-password request for Super Admin: <strong>${user.username}</strong></p><p>Password is stored hashed. Log in and use Change Password, or resolve from admin panel.</p>`,
          });
          return NextResponse.json({ success: true, message: genericOk });
        } catch (e: any) {
      ...
      // Non-super: record reset request
    ...
    if (action === "list_reset_requests") {
      ...
      const list = await getJsonSetting("password_reset_requests", []);
    ...
    if (action === "resolve_reset") {
      ...
      if (newPassword && userId) {
        const hashed = await bcrypt.hash(String(newPassword), 10);
        await db.update(users).set({ passwordHash: hashed }).where(eq(users.id, Number(userId)));
      }
      if (requestId) {
        const key = "password_reset_requests";
        const existing = await db.select().from(settings).where(eq(settings.key, key)).limit(1);
        if (existing.length) {
          ...
          await db.update(settings).set({ value: JSON.stringify(list) }).where(eq(settings.key, key));
        }
      }
